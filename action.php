...
if(isset($_POST["action"]))
{
		...
		if($object->row_count() > 0)
		{
			...
			foreach($result as $row)
			{
				if($row["email_verify"] == 'No')
				{
					...
					{
						$error = '<div class="alert alert-danger">Yanlış Şifre</div>';
					}
				}
				else
				{
	...
	if($_POST['action'] == 'edit_profile')
	{
		...
		$_SESSION['success_message'] = '<div class="alert alert-success">Profi Güncellendi</div>';
	...
	if($_POST['action'] == 'make_appointment')
	{
		...
		foreach($patient_data as $patient_row)
		{
			$html .= '
			<tr>
				<th width="40%" class="text-right">Hasta İsmi</th>
				<td>'.$patient_row["patient_first_name"].' '.$patient_row["patient_last_name"].'</td>
			</tr>
			<tr>
				<th width="40%" class="text-right">Tel No.</th>
				<td>'.$patient_row["patient_phone_no"].'</td>
			</tr>
			<tr>
				<th width="40%" class="text-right">Adres</th>
		...
		<hr />
		<h4 class="text-center">Randevu Detayları</h4>
		<table class="table">
		';
		foreach($doctor_schedule_data as $doctor_schedule_row)
		{
			$html .= '
			<tr>
				<th width="40%" class="text-right">Doktor İsmi</th>
				<td>'.$doctor_schedule_row["doctor_name"].'</td>
			</tr>
			<tr>
				<th width="40%" class="text-right">Randevu Tarihi</th>
				<td>'.$doctor_schedule_row["doctor_schedule_date"].'</td>
			</tr>
			<tr>
				<th width="40%" class="text-right">Randevu Günü</th>
				<td>'.$doctor_schedule_row["doctor_schedule_day"].'</td>
			</tr>
			<tr>
				<th width="40%" class="text-right">Müsait Zaman</th>
	...
	if($_POST['action'] == 'book_appointment')
	{
		...
		if($object->row_count() > 0)
		{
			$error = '<div class="alert alert-danger">Bugünlük Randevunuz zaten var.Öbür günler için Deneyin.</div>';
		}
		else
		{
			$object->query = "
	...
	if($_POST['action'] == 'cancel_appointment')
	{
		...
		";
		$object->execute($data);
		echo '<div class="alert alert-success">Randevunuz İptal Edildi.</div>';
	}
}
